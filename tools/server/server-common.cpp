...
server_tokens process_mtmd_prompt(mtmd_context * mctx, const std::string & prompt, const std::vector<raw_buffer> & files, bool is_placeholder, const std::vector<video_frame_meta> & video_frames) {
    ...
    for (size_t i = 0; i < files.size(); i++) {
        ...
        for (const auto & vf : video_frames) {
            if (vf.file_idx == (int)i) {
                mtmd_bitmap_set_video_frame(out.bitmap, vf.frame_idx, vf.n_frames_total, vf.timestamp_sec);
                break;
            }
        }
