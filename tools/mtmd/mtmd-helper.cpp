...
int32_t mtmd_helper_decode_image_chunk(
    ...
    if (mtmd_decode_use_mrope(ctx)) {
        if (chunk_type == MTMD_INPUT_CHUNK_TYPE_IMAGE) {
            ...
            const int n_frames = mtmd_image_tokens_get_n_frames_total(image_tokens);
            const int frame_idx = mtmd_image_tokens_get_frame_idx(image_tokens);
            if (n_frames > 0) {
                // Video frame: use 3D positions (temporal + spatial)
                batch_embd.set_position_mrope_3d(n_past, nx, ny, frame_idx, seq_id);
            } else {
                // Static image: use 2D positions (spatial only)
